...
pfit <-
  prune(fitall,
        cp = fitall$cptable[which.min(fitall$cptable[, "xerror"]),
                            "CP"])
...
prune_fitk <- 
  prune(fitk, 
        cp = fitk$cptable[which.min(fitk$cptable[, "xerror"]),
                          "CP"])
class(prune_fitk)
...
peoplec <-
  people %>% 
  select(Diabetes, Age, BMI, Gender, PhysActive) %>%
  mutate(fdiab = as.factor(Diabetes))
summary(peoplec)
...
```{r}
diabrf <- rfsrc(fdiab ~ Age + Gender + BMI + PhysActive, 
                data = peoplec, 
                ntree = 1000, 
                tree.err = TRUE)
